...
export PRIMARY_NIC=env2
export FIRST_SECONDARY_NIC=env5
export SECOND_SECONDARY_NIC=env6
export DNS_TEST_NIC=env5
...
if oc get ns openshift-ovn-kubernetes &> /dev/null; then
    ...
    export PRIMARY_NIC=env2
    # env2 is owned by OVN/OVS br-ex on this OCP environment.
    # Do not allow the handler reset path to reconfigure the primary NIC.
    export SKIP_PRIMARY_NIC_CONFIG=TRUE
